...
function crearConfetiParticulas() {
    const { particles } = getElements();
    particles.innerHTML = '';
    for (let i = 0; i < 30; i++) {
        const particula = createElement('div', 'fixed w-2 h-2 bg-rose-300 rounded-full pointer-events-none animate-fall');
...
function renderTrivia() {
    ...
    const progreso = ((state.triviaActual + 1) / CONFIG.preguntasTrivia.length) * 100;
...
function responderTrivia(opcionIndex) {
    const pregunta = CONFIG.preguntasTrivia[state.triviaActual];
    if (opcionIndex === pregunta.respuesta) {
      ...
      `;
        ...
        const botonIncorrecto = document.getElementById(`opcion-${opcionIndex}`);
        if (botonIncorrecto) {
